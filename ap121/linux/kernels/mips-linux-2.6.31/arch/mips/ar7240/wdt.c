...
#define UDELAY_COUNT 4000
	...
	if (!delay) {
		//printk("%s %d: watch dog intriggered.\n", __FUNCTION__,__LINE__);
#ifdef CONFIG_MACH_HORNET
	...
	//printk("%s %d: watch dog not intriggered.\n", __FUNCTION__,__LINE__);
	return IRQ_HANDLED;
}
...
#if 0
	/* disable ar7240wdt_init, by zcf */
	printk("====>intentionally disable ar7240wdt.\n");
	reg_val = ar7240_reg_rd(ETHSW_CLOCK);
	//printk("before: ETHSW_CLOCK register value = 0x%x\n", reg_val);
	reg_val |= (1<<3);
	reg_val &= ~(1<<4);
	ar7240_reg_wr(ETHSW_CLOCK, reg_val);
	//reg_val = ar7240_reg_rd(ETHSW_CLOCK);
	//printk("after: ETHSW_CLOCK register value = 0x%x\n", reg_val);
	return;
#endif	
