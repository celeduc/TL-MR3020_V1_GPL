...
static int __init ar7240_flash_init (void)
{
	...
	mtd->flags		=   MTD_CAP_NORFLASH | MTD_WRITEABLE;
    	...
    	flashid = ar7240_spi_read_cmd(0x9f) >> 8;
        if (flashid == 0x1c3017)
        {
            mtd->size = AR7240_FLASH_SIZE_8MB;
            rwBuf = (unsigned char *)kmalloc(AR7240_FLASH_SECTOR_SIZE_64KB, GFP_KERNEL); 
            memset(rwBuf, 0, AR7240_FLASH_SECTOR_SIZE_64KB);
			ar7240_flash_read(mtd, 0x3f0000, AR7240_FLASH_SECTOR_SIZE_64KB, &retlen, rwBuf);

            if (rwBuf[0] == 0xff && rwBuf[1] == 0xff && rwBuf[2] == 0xff && rwBuf[3] == 0xff)
            {
                printk("art data has been copied\n");
            }
            else
            {
                ar7240_flash_read(mtd, 0x7f0000, AR7240_FLASH_SECTOR_SIZE_64KB, &retlen, rwBuf);
                ar7240_spi_sector_erase(0x3f0000);
                ar7240_flash_write(mtd, 0x3f0000, AR7240_FLASH_SECTOR_SIZE_64KB, &retlen, rwBuf);

                memset(rwBuf, 0, AR7240_FLASH_SECTOR_SIZE_64KB);
                ar7240_flash_read(mtd, 0x7e0000, AR7240_FLASH_SECTOR_SIZE_64KB, &retlen, rwBuf);
                ar7240_spi_sector_erase(0x3e0000);
                ar7240_flash_write(mtd, 0x3e0000, AR7240_FLASH_SECTOR_SIZE_64KB, &retlen, rwBuf);
            }
            kfree(rwBuf);
        }        
