...
/* [liaoxingwei start] fix bug#45679: PPPOE dial exception when get inpacket which is not for me*/
...
/************************************************************************
 *
 * Adjust whether the pppoe pkt is llegal or not.
 *
 ***********************************************************************/
 static bool pppoe_is_legal(struct sk_buff *skb, struct pppox_sock *po)
 {
	char my_mac[32] = {0};
	char remote_mac[32] = {0};
	char myEth[32] = {0};
	char peerEth[32] = {0};
	...
	sprintf(my_mac, "%02X:%02X:%02X:%02X:%02X:%02X",
		(unsigned) eth->h_dest[0],
		(unsigned) eth->h_dest[1],
		(unsigned) eth->h_dest[2],
		(unsigned) eth->h_dest[3],
		(unsigned) eth->h_dest[4],
		(unsigned) eth->h_dest[5]);

	sprintf(remote_mac, "%02X:%02X:%02X:%02X:%02X:%02X",
		(unsigned) eth->h_source[0],
		(unsigned) eth->h_source[1],
		(unsigned) eth->h_source[2],
		(unsigned) eth->h_source[3],
		(unsigned) eth->h_source[4],
		(unsigned) eth->h_source[5]);

	if (po->pppoe_dev->dev_addr != NULL)
	{
		sprintf(myEth, "%02X:%02X:%02X:%02X:%02X:%02X",
			(unsigned) po->pppoe_dev->dev_addr[0],
			(unsigned) po->pppoe_dev->dev_addr[1],
			(unsigned) po->pppoe_dev->dev_addr[2],
			(unsigned) po->pppoe_dev->dev_addr[3],
			(unsigned) po->pppoe_dev->dev_addr[4],
			(unsigned) po->pppoe_dev->dev_addr[5]);
	}

	sprintf(peerEth, "%02X:%02X:%02X:%02X:%02X:%02X",
		(unsigned) po->pppoe_pa.remote[0],
		(unsigned) po->pppoe_pa.remote[1],
		(unsigned) po->pppoe_pa.remote[2],
		(unsigned) po->pppoe_pa.remote[3],
		(unsigned) po->pppoe_pa.remote[4],
		(unsigned) po->pppoe_pa.remote[5]);

	/* adjust whether the protocol type, my mac and remote mac is right. */
	if ((eth->h_proto != ETH_PPPOE_SESSION && eth->h_proto != ETH_PPPOE_DISCOVERY)
		|| strcmp(my_mac, myEth) != 0 || strcmp(remote_mac, peerEth) != 0)
	{
		printk("PPPOE packet is illegal\n");
		return false;
	}
		     ...
		     struct packet_type *pt, struct net_device *orig_dev)
{
	...
	/* [liaoxingwei start] */
	if (pppoe_is_legal(skb, po) == false)
	{
		goto drop;
	}
	/* [liaoxingwei end] */
