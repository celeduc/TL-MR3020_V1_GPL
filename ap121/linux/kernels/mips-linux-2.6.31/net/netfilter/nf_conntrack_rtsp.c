                ...
                struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	while (dataoff < datalen) {
		...
		if (expinfo.pbtype == pb_range) {
			...
			/*start added by huanglifu,2012-05-31.For fix up RTSP ALG fail. */
			//exp->mask.src.u.udp.port  = 0xfffe;
			exp->mask.src.u.udp.port  = 0;
			/* end added by huanglifu, 2012-05-31 */
		}
		...
		struct nf_conn *ct, enum ip_conntrack_info ctinfo)
{
	...
	/*start changed by huanglifu,2012-05-31.For fix up RTSP ALG fail. */
 	//if (ctinfo != IP_CT_ESTABLISHED &&
 	if (ctinfo != IP_CT_NEW && ctinfo != IP_CT_ESTABLISHED &&
	/* end changed by huanglifu, 2012-05-31 */
