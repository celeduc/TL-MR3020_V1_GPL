...
{
	memmove(buf+match_offset+rep_len+diff_offset,  buf+match_offset+match_len+diff_offset,  buflen-match_offset-match_len);
                 ...
                 struct sk_buff* skb, uint tranoff, uint tranlen)
{
    ...
    while (off < tranlen)
    {
        ...
        while (off < nextparamoff)
        {
            ...
            if (dstact != DSTACT_NONE && strncmp(ptran+off, "destination=", 12) == 0)
            {
                ...
                mangle_tran_buf(tran_buf, tranbuf_len, tranbuf_diff, off+12, diff-12-1, temaddr, tmplen);
        ...
        while (off < nextparamoff)
        {
			...
			nf_ct_unexpect_related(exp);
                    ...
                    #endif
                    mangle_tran_buf(tran_buf, tranbuf_len,  tranbuf_diff, origoff, origlen, rbuf, rbuflen);
    ...
    nf_nat_mangle_tcp_packet(skb, ct, ctinfo, tranoff, tranbuf_len, tran_buf, tranbuf_len + tranbuf_diff);
	 ...
	 struct nf_conntrack_expect* exp)
{
    ...
    printk("[RTSPAlg] NAT rtsp help out\n");
