...
int main(int argc, char **argv, char **envp)
{
	...
	//这里的定义了PPTP_DEBUG之后如果想在串口得到打印信息，
	//pppd也必须指定PPPD_DEBUG才可以，因为pptp进程是在pppd中启动的
	#ifdef PPTP_DEBUG
	debug = 1;
	log_level = 1;
	#endif
...
int get_call_id(int sock, pid_t gre, pid_t pppd, 
		 u_int16_t *call_id, u_int16_t *peer_call_id)
{
	//log("YangXv:get_call_id---callmrg_sock=%d parent_pid=%d
	//			child_pid=%d", sock, gre, pppd);
     ...
     */
     //log("YangXv:call_id=%d  peer_call_id=%d", m_call_id, m_peer_call_id);
    *call_id = m_call_id;
