...
parse_string(const char *s, struct xt_string_info *info)
{
    ...
    do
	{
		...
		if (strlen(prev) >= XT_STRING_MAX_PATTERN_SIZE)
			xtables_error(PARAMETER_PROBLEM, "string match: too long '%s'\n", prev);
		
		if (count >= XT_STRING_MAX_NUM)
			xtables_error(PARAMETER_PROBLEM, "string match: too many string specified (MAX = %d)\n", XT_STRING_MAX_NUM);
		
		strncpy(info->pattern[count], prev, strlen(prev));
		info->patlen[count] = strlen(prev);

		//printf("multistring_debug: string_%d, %s, %d\n", count, info->pattern[count], info->patlen[count]);
...
string_print(const void *ip, const struct xt_entry_match *match, int numeric)
{
	...
	for(i = 0; i < info->string_count; ++i)
	{
		if (is_hex_string(info->pattern[i], info->patlen[i])){
			if (i == 0)
				printf("STRING match %s", invert ? "!" : "");
			print_hex_string(info->pattern[i], info->patlen[i]);
		} else {
			if (i == 0)
				printf("STRING match %s", invert ? "!" : "");
			print_string(info->pattern[i], info->patlen[i]);
		}
	}
