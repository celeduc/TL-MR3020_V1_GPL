...
static struct option long_options[] = {
    ...
    {"ctrlmsg-content",		required_argument, 0, 'X'},
...
void readConfigFile(const char *configFilename)
{
	...
	ParseParamString(configFilename, CtrlmsgContent);
...
int readArguments(int argc, char **argv)
{
	...
	while (1)
	{
		c = getopt_long (argc, argv, "heWQDndHSOGTNALRIv:p:V:P:C:m:M:2:3:w:r:c:i:u:a:s:X:",
		...
		switch (c)
		{
			...
			case 'X': strcpy(CtrlmsgContent, optarg); break;
...
int main(int argc, char **argv)
{
	...
	if (strlen(MessageContent)) {
		...
		if (strstr(MessageContent,"-") == NULL && hexstr2bin(MessageContent, ByteString, strlen(MessageContent)/2) == -1) {
			fprintf(stderr, "Error: MessageContent %s\n is not a hex string. Aborting.\n\n", MessageContent);
			exit(1);
		}
	}
	SHOW_PROGRESS("\n");
    printf("CtrlmsgContent is %s\n", CtrlmsgContent);
	if (show_progress)
	...
	if ( !specialMode && !strlen(MessageContent) && AltSetting == -1 && Configuration == 0 && !strlen(CtrlmsgContent))
    ...
    if (strlen(CtrlmsgContent))
    {
        switchCtrlmsgContent(CtrlmsgContent);
    }
...
void switchCtrlmsgContent (char *CtrlmsgContent)
{
	int ret;
    int message_length = 0;
    unsigned int bRequestType, bRequest, wValue, wIndex, wLength;
    if (CtrlmsgContent == NULL)
        return;
    
	message_length = strlen(CtrlmsgContent) / 2;
	if ( hexstr2bin(CtrlmsgContent, ByteString, message_length) == -1) 
    {
        printf("====>>>>control msg error\n");
        return;
	}    
    bRequestType = (unsigned char)ByteString[0];
    bRequest = (unsigned char)ByteString[1];
    wValue = (unsigned int)ByteString[3]*0xff + (unsigned int)ByteString[2];
    wIndex = (unsigned int)ByteString[5]*0xff + (unsigned int)ByteString[4];
    wLength= (unsigned int)ByteString[7]*0xff + (unsigned int)ByteString[6];
    printf("bRequestType is 0x%x bRequest is 0x%x wValue is 0x%x wIndex is 0x%x wLength is 0x%x\n", bRequestType, bRequest, wValue, wIndex, wLength);
	SHOW_PROGRESS("====>>>>Trying to send unknown control message\n");
	ret = usb_control_msg(devh, bRequestType, bRequest, wValue, wIndex, buffer, wLength, 1000);
	if (ret != 0) {
		fprintf(stderr, "Error: sending unknown control message failed (error %d). Aborting.\n\n", ret);
	    exit(1);
	} else
		SHOW_PROGRESS(" OK, unknown control message sent\n");
}
