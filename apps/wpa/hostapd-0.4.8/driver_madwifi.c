...
static int
/*
 * Aug 07, 2008, modified by Liang Xin.
 * Add a special vendor IE before wsc ie with WEP and WSC on.
 * This special vendor IE can make vista client not sending EPAOL-Start
 * automatically.
 */
madwifi_set_wsc_ie(void *priv, u8 *iebuf, int iebuflen, u32 frametype)
{
    ...
    struct hostapd_config *conf = hapd->conf;
	u8 buf[256];
    u8 wep_wps_ie[] = {IEEE80211_ELEMID_VENDOR, 0x05, 0x00, 0x50, 0xf2,
                        0x05, 0x00 };
	struct ieee80211req_getset_appiebuf * beac_ie;

	HOSTAPD_DEBUG(HOSTAPD_DEBUG_MINIMAL, "%s buflen = %d if=%s\n",
			__func__, iebuflen, drv->iface);

	beac_ie = (struct ieee80211req_getset_appiebuf *) buf;
	beac_ie->app_frmtype = frametype;

    /* Modified by Liang Xin, Aug 07, 2008 */
    ...
    {
        beac_ie->app_buflen = iebuflen + sizeof(wep_wps_ie);
        memcpy(&(beac_ie->app_buf[0]), wep_wps_ie, sizeof(wep_wps_ie));
        memcpy(&(beac_ie->app_buf[sizeof(wep_wps_ie)]), iebuf, iebuflen);
    }
...
madwifi_new_sta(struct madwifi_driver_data *drv, u8 addr[IEEE80211_ADDR_LEN])
{
	...
	if (hapd->conf->wpa) {
		...
		sta->wpa_ie = NULL;
		sta->wpa_ie_len = 0;
        // why not allow WEP??? -- liangxin
                if(hapd->conf->wsc_auth_mode == HOSTAPD_AUTH_OPEN
                        || hapd->conf->wsc_auth_mode == WSC_AUTH_MODE_WEP)
                {
