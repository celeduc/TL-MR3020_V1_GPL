...
#ifdef EAP_WSC
        if( state->hapd->conf->wsc_auth_mode ) {
		if (state->respTimeout > 0) {
			state->respTimeout--;
			if (state->respTimeout == 0) {
				wpa_printf(MSG_DEBUG, "IEEE 802.1X: " MACSTR
					   " - respTimeout --> 0",
					   MAC2STR(state->addr));
				state->eapTimeout = TRUE;
			}
			else if ( !(state->respTimeout % 5) ) {  // 10, 5
				wpa_printf(MSG_DEBUG, 
					"IEEE 802.1X:respTimeout retransmit");
				state->be_auth.eapReq = TRUE;
			}
		}
	}	
#endif
...
SM_STATE(AUTH_PAE, DISCONNECTED)
{
	...
	sm->authPortStatus = Unauthorized;
        if ((sm->hapd->conf->wsc_auth_mode != HOSTAPD_AUTH_OPEN ) &&
            (sm->hapd->conf->wsc_auth_mode != WSC_AUTH_MODE_WEP))
        {
                /* Jerry do not unauthorize the port for open and WEP network*/
	        setPortUnauthorized();
        }
...
#ifdef EAP_WSC
	char tmpBuf[11+SIZE_MAC_ADDR+1];
        if( sm->hapd->conf->wsc_auth_mode ) {
		sprintf( tmpBuf, "STA_TIMEOUT:");
		memcpy(&tmpBuf[11], sm->sta->addr, SIZE_MAC_ADDR);
		wsc_event_notify(tmpBuf);
	}
#endif
	sm->authTimeout = TRUE;
}
...
#if 0 /* temply turn off this function. Since STA has a bug, it will de-auth with AP when wsc running */
#ifdef EAP_WSC
	char tmpBuf[11+SIZE_MAC_ADDR+1];
        if( sm->hapd->conf->wsc_auth_mode ) {
		sprintf( tmpBuf, "STA_REMOVE:");
		memcpy(&tmpBuf[11], sm->sta->addr, SIZE_MAC_ADDR);
		wsc_event_notify(tmpBuf);
	}	
#endif
...
restart:
	...
	if (prev_auth_pae != sm->auth_pae.state ||
        ...
        /* WSC should wait station send EAPOL_START */
        if ((sm->hapd->conf->wsc_auth_mode == 0) || 
            ((sm->hapd->conf->wsc_auth_mode) && 
                   (sm->sta->flags & WLAN_STA_WSC_START_RCVD))) 
        {    
...
#ifdef EAP_WSC
static void eapol_sm_set_eap_respTimeout(void *ctx, int value)
{
	struct eapol_state_machine *sm = ctx;
	if (sm == NULL)
		return;

	sm->respTimeout = value;
}
#endif
...
#ifdef EAP_WSC
	.set_eap_respTimeout = eapol_sm_set_eap_respTimeout,
#endif
};
