...
#ifdef EAP_WSC
static void eapol_set_respTimeout(struct eap_sm *sm, int value)
{
	sm->eapol_cb->set_eap_respTimeout(sm->eapol_ctx, value);
}
#endif
...
#ifdef EAP_WSC
        if( sm->sta->eapol_sm->hapd->conf->wsc_auth_mode ) {
		eapol_set_respTimeout(sm, 0);
	}
#endif
}
...
static int eap_sm_Policy_getDecision(struct eap_sm *sm)
{
	...
	if ((sm->user == NULL || sm->update_user) && sm->identity) {
		if (eap_user_get(sm, sm->identity, sm->identity_len, 0) != 0) {
			...
			int iii = 0;
			printf("users:     ");
			for (iii = 0; iii < sm->identity_len; iii ++)
			{
				printf("0x%02x ", sm->identity[iii]);
			}
			printf("\n\n\n");
#else
			printf("\n\n\nusers: %s\n\n\n", sm->identity);
