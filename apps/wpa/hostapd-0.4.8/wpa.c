...
//10Feb07, add by dqw.
//fixed the mixed wpa authentication by sending a EAPOL package forcibly.
	u8 rsc[8];
	struct wpa_authenticator *gsm = sm->hapd->wpa_auth;
	hostapd_get_seqnum(sm->hapd, NULL, sm->hapd->wpa_auth->GN, rsc);
	wpa_send_eapol(sm->hapd, sm->sta, 1, 1, 1, !sm->Pair, 0, rsc,
		       gsm->GNonce, NULL, 0, gsm->GTK[gsm->GN - 1],
		       gsm->GTK_len, gsm->GN);
}
...
#ifdef WSC_AKM_NO_1X
    /*check WLAN_STA_WSC_START_RCVD flag only if wsc_auth_mode is enabled*/
    if(((hapd->conf->wsc_auth_mode !=0) && 
       (sm->sta->flags & WLAN_STA_WSC_ASSOC) ) ||
       (sm->sta->flags & WLAN_STA_WSC_REG_ID_RCVD) ||
       (sm->sta->flags & WLAN_STA_WSC_STA_ID_RCVD) ||
       (sm->sta->flags & WLAN_STA_WSC_START_PATCH_RCVD))
    {
        HOSTAPD_DEBUG(HOSTAPD_DEBUG_MINIMAL, "WPA: group state machine "
		      "topped\n");
        return;
