...
static int bIsNobufferInXmit = 0;
static int bNeedStopRecvPkt = 0;
static unsigned long noBufferTimeExpires = 0;
...
static void pppol2tp_recv_dequeue_skb(struct pppol2tp_session *session, struct sk_buff *skb)
{
	 ...
	 */
#if 0	/* move to before call this function  */ 
	...
	if (session_sock->sk_state & PPPOX_BOUND) {
		...
		PRINTK(session->debug, PPPOL2TP_MSG_DATA, KERN_INFO,
		       "%s: socket not bound\n", session->name);
/* learn from pppol2tp.c in kernel 2.6.30, by lqt */
#if 1
...
/* add missing sock_put() in pppol2tp_recv_dequeue(),
...
/* goto out should be include spin_unlock, by lqt */
out:
	spin_unlock(&session->reorder_q.lock);
//out:
	EXIT_FUNCTION;
...
//	static unsigned int	freeSkbCount = 0;
	...
	if (PPPOL2TP_SKB_CB(skb)->has_seq) {
		...
		// no buffer while xmit, drop some pkt, by lqt
		if(1 == bNeedStopRecvPkt)
		{			
			/*
			struct pppox_sock *po;			
			session_sock = session->sock;
			...
			*/
		#if 0
			if(freeSkbCount++ % 128 == 0)
			{
				STR_PRINTK("recv reset nobuf\n");
				bNeedStopRecvPkt = 0;
			}
		#else
			if(time_after(jiffies, noBufferTimeExpires))
			{
				STR_PRINTK("recv reset nobuf\n");
				bNeedStopRecvPkt = 0;
			}
		#endif
			/* only drop ip pkt  */
			if((skb->data[0] == 0 && skb->data[1] == 0x21) || skb->data[0] == 0x21)
			{
				STR_PRINTK("lqt rcv drop ip\n");
				goto discard;
			}
		}
...
#if 1
	/* drop 8 pkts while not buffer, by lqt, 08.04 */
	/* the more better method is let ppp slow down to send pkt, 
	 * but i dont how to do now, have not see the interface funciton. */
	if(1==bIsNobufferInXmit)
	{
#if 0
		if(freeSkbCount++ % 128 == 0)
	{
			STR_PRINTK("xmit reset nobuf\n");
			bIsNobufferInXmit = 0;
		}
	#else /* use pass time */
		if(time_after(jiffies, noBufferTimeExpires))
...
#endif
		/* only drop ip pkt, not control pkt */
		if((skb->data[0] == 0 && skb->data[1] == 0x21) || skb->data[0] == 0x21 )
		{
			STR_PRINTK("lqt xmit drop ip\n");
			goto end;
		}
	}
#endif
...
//	printk("err:%d\n", error);	
	...
	/* fix memory leak, add by lqt, 2010.7.16 */
	if(skb)
		kfree_skb(skb);
	...
	if(-ENOBUFS == error)
	{
		/* while no buffer, not recv and xmit pkts in 1s */
		noBufferTimeExpires = jiffies + HZ/4;
		bIsNobufferInXmit = 1;
		bNeedStopRecvPkt = 1;
		
		// make ppp slow to send?
		//return 0;
	}
	return 1;
//	return error;
...
/* add missing sock_put() in pppol2tp_tunnel_closeall() */			
...
/* delete this, while httpd -k, this can case 
...
/* fix bug, remove null pointer dereference */
#if 0
	if (error != 0)
...
#endif /* CONFIG_PROC_FS */
	printk(KERN_INFO "PPPoL2TP kernel driver, %s. fixed many bug. drop rcv pkts and xmit pkts while no buffer, use pass time HZ/4.\
only drop ip pkt. no printk. xl2tpd 15 retries xmit. 08.20\n", PPPOL2TP_DRV_VERSION);
